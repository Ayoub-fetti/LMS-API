  ...
  async getStudentsProgress(
    ...
    const studentsProgress = await Promise.all(
      enrollments.map(async (enrollment) => {
        ...
        const courseQuizSubmissions = quizSubmissions.filter(
          (sub) => sub.quiz !== null,
        );
        ...
        const moduleStats = modules.map((module) => {
          ...
          const isCurrent =
            progress?.currentModule?._id.toString() === module._id.toString();
    ...
    const globalStats = {
      ...
      averageCompletion:
        studentsProgress.length > 0
          ...
          : 0,
      studentsCompleted: studentsProgress.filter(
        (s) => s.progress.completionPercentage === 100,
      ).length,
      studentsInProgress: studentsProgress.filter(
        (s) =>
          s.progress.completionPercentage > 0 &&
          s.progress.completionPercentage < 100,
      ).length,
      studentsNotStarted: studentsProgress.filter(
        (s) => s.progress.completionPercentage === 0,
      ).length,
      averageQuizScore:
        studentsProgress.length > 0
          ? Math.round(
              studentsProgress.reduce(
                (sum, s) => sum + s.quizzes.averageScore,
                0,
              ) / studentsProgress.length,
            )
          : 0,
